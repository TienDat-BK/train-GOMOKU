...
class IncrementalModel(nn.Module):
    def __init__(self,in_channels=32):
        ...
        self.resBlock = nn.Sequential(ResBlock(in_channels),
...
class extract_policy(nn.Module):
    def __init__(self, in_channels):
        ...
        self.conv_7_5_3_1 = nn.Sequential(nn.Conv2d(in_channels=in_channels,out_channels=in_channels,kernel_size=7,padding=3),
                                          nn.BatchNorm2d(in_channels),
                                          nn.ReLU(in_channels),
                                          nn.Conv2d(in_channels=in_channels,out_channels=in_channels,kernel_size=5,padding=2),
                                          nn.BatchNorm2d(in_channels),
                                          nn.ReLU(in_channels),
                                          nn.Conv2d(in_channels=in_channels,out_channels=in_channels,kernel_size=3,padding=1),
                                          nn.BatchNorm2d(in_channels),
                                          nn.ReLU(in_channels),
                                          nn.Conv2d(in_channels=in_channels,out_channels=1,kernel_size=1,padding=0),
                                          nn.ReLU(1),)
    
    def forward(self,x):
        # input (B,in_channels,15,15)
        x = self.conv_7_5_3_1(x)
        # out (B,1,15,15)
        return x
...
class MyModel(nn.Module):
    def __init__(self):
        ...
        # Input: (B, 3, 15, 15)
        self.extractDirFeatures = ExtractDirectionalFeatures(in_channels=3, mid_channels=32, out_channels=64, kernel_size=3, padding=1)
        self.extractPolicy = extract_policy(64*4)

    def forward(self, x):
        # x: (B, 3, 15, 15)

        x = self.extractDirFeatures(x)   # (B, 64, 15, 15)
        x = self.extractPolicy(x)
        x = x.view(x.size(0),-1)
    ...
    def train_model(self, dataloader):
        ...
        optimizer = torch.optim.Adam([
            ...
            {"params": self.extractPolicy.parameters(), "lr": 0.002}  # phần policy nhẹ hơn
        ])
        ...
        for epoch in range(4000):
            ...
            for data_input, target_probs in dataloader:
                ...
                if loss < 0.1:
                    ...
                    torch.save(self.state_dict(), f"model_rapfi_save_2.pth")
